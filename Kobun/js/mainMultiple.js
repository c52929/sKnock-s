'use strict';
{
	...
	function newQuestion(){
		...
		*/
		...
		if(words[rs[0]].charAt(2)+words[rs[0]].charAt(3)=="あや"){
			showRuby=true;
		}
	...
	function reset(sedQ,orz){
		...
		if(sedQ[2]){
			bengiW[1].push("うつくし","かなし","いみじ","をかし","よし","奇:あや;し・怪:あや;し","賤:あや;し","{大人}{おとな}し","ゆかし","おぼつかなし","有:あ;り難:がた;し","めでたし","口:くち;惜:を;し","憂:う;し","侘:わ;びし");
			bengiM[1].push("かわいらしい/立派だ","いとしい/かわいい","とても/とてもすばらしい/とても悲しい","趣がある/美しい/滑稽だ","よい/身分が高く教養がある","不思議だ/けしからん","身分が低い/粗末だ","大人らしい/年配だ/思慮分別がある","見たい/聞きたい/知りたい","ぼんやりしている/気がかりだ/待ち遠しい","めったにない/(めったにないほど)立派だ","すばらしい","残念だ","つらい","つらい/興ざめだ");
		}
		if(sedQ[3]){
		...
		if(sedQ[5]){
			bengiW[4].push("げに","なほ","やがて","やうやう","え～打消","な～そ","さらに～打消し","いかで・いかでか","いと","あまた");
		...
		if(sedQ[7]){
			bengiW[1].push("おもしろし","あらまほし","やむごとなし","ゆゆし","畏:かしこ;し・恐:かしこ;し","賢:かしこ;し","うるはし","はかなし","はかばかし","やさし","いはけなし","かたはらいたし","心:こころ;もとなし","いとほし","あさまし","いたし","つれなし","あいなし","あぢきなし","さがなし","すさまじ","言:い;ふ甲:か;斐:ひ;無:な;し","よしなし","便:びん;無し","さうざうし","むつかし");
			bengiM[1].push("すばらしい/興趣がある","理想的だ","高貴だ/この上ない","不吉だ/すばらしい/はなはだしい","おそれ多い/非常に","すばらしい/非常に","端正だ/親しい","頼りない/ちょっとした","しっかりしている/はっきりしている","優雅だ/けなげだ","幼い","見苦しい/気の毒だ/恥ずかしい/ばかばかしい","待ち遠しい/不安だ/はっきりしない","かわいそうだ/かわいい","驚きあきれるほどだ/あきれるほどひどい/情けない","とても/たいして(～ない)/すばらしい","平然としている/冷淡だ/何の変化もない","筋違いだ/つまらない/むやみに","どうにもならない/つまらない","性質がよくない/いたずらだ","興ざめだ/殺風景だ","どうしようもない/取るに足りない/ひどい","関係がない/方法がない/つまらない","都合が悪い/困ったことだ","物足りない","うっとうしい/わずらわしい/むさくるしい");
		}
		if(sedQ[8]){
			bengiW[2].push("貴:あて;なり","清:きよ;らなり","更:さら;なり","顕:あら;はなり","あからさまなり","頑:かたく;ななり","徒:あだ;なり","漫:すず;ろなり","無:む;下:げ;なり");
			bengiM[2].push("身分が高い/上品だ/優美だ","美しい","(いまさら)言うまでもない","まる見えだ/明らかだ","ちょっとの間だ","情趣を解さない/見苦しい","はかない/不誠実だ","あてもない/思いがけない/むやみに","まったくひどい/むやみに/まったく(～ない)");
		}
		if(sedQ[9]){
			bengiW[3].push("公:おほやけ;","際:きは;","験:しるし;・徴:しるし;","理:ことわり;","例:ためし;","業:わざ;","覚:おぼ;え","心:ここ;地:ち;","現:うつつ;","つとめて","晦:つごもり;・晦日:つごもり;","ついで","頼:たよ;り・便:たよ;り","志:こころざし;","由:よし;","万:よろづ;","手:て;","才:ざえ");
			bengiM[3].push("朝廷/天皇","身分/程度","御利益/効果/前兆","道理","例/語り草","仏事/こと","評判/寵愛(を受けること)","気持ち/病気","現実/正気","早朝/翌朝","月末","順序/機会","頼れるもの/縁故/便宜/機会","愛情/贈り物","縁/方法/由緒・風情/事情/そぶり","さまざまなこと/すべて","筆跡/演奏法","学問");
		}
		if(sedQ[10]){
			bengiW[4].push("自:おのづか;ら","即:すなは;ち・則:すなは;ち","疾:と;く","いつしか","なべて","いとど","かく","とかく","しか","さ","さばかり","さながら","つゆ～打消","よも～じ","なかなか","さすがに","かつ","など・などか","なでふ・なんでふ");
		...
		if(sedQ[12]){
			bengiW[6].push("宣:のたま;ふ・宣:のたま;はす","仰:おほ;す","聞:き;こし召:め;す","御:ご;覧:らん;ず","申:まう;す","聞:き;こゆ・聞:き;こえさす","おはす・おはします","思:おぼ;す・思:おぼ;し召:め;す","賜:たま;ふ・給:たま;ふ(ハ行四段)","賜:たま;ふ・給:たま;ふ(ハ行下二段)","賜:たま;はす","賜:たま;はる・給:たま;はる","承:うけたまは;る","奉:たてまつ;る","侍:はべ;り","候:さぶら;ふ","召:め;す","詣:まう;づ","参:まゐ;る","参:まゐ;らす","罷:まか;る","罷:まか;づ","遣:つか;はす","遊:あそ;ばす","仕:つか;うまつる","大:おお;殿:との;籠:ごも;る","しろしめす","行:ぎやう;幸:がう;・行:ぎやう;啓:けい;・御:ご;幸:かう;","奏:そう;す","啓:けい;す");
		...
		if(sedQ[15]){
			bengiW[2].push("強:あなが;ちなり","なめげなり","なのめなり","おぼろげなり","あやにくなり","まめやかなり","切:せち;なり","よそなり","うちつけなり","頓:とみ;なり","密:みそ;かなり","こまやかなり","らうたげなり","優:いう;なり");
		...
		if(sedQ[17]){
			bengiW[4].push("互:かたみ;に","わざと(の)","うたて","予:か;ねて","やをら","夜:よ;もすがら","ゆめ・ゆめゆめ～打消・禁止","をさをさ～打消");
		...
		if(sedQ[20]){
			bengiW[1].push("労:らう;労:らう;じ","むくつけし","おほけなし","いぶせし","著:しる;し","こよなし","悔:くや;し","楽:たの;し");
		...
		if(sedQ[23]){
			bengiW[4].push("折:をり;節:ふし;","早:はや;く","つくづくと","せめて","さりとも","さて","おほかた～打消","かまへて");
